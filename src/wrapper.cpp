...
namespace Clusters {
  ...
  int Wrapper::load_parameters(IOArray &modelpars) {
    ...
    nuisance.load_parameters(modelpars[13], modelpars[14], modelpars[15], modelpars[16], modelpars[17], modelpars[18]);
    int i = 19;
    for (int j=0; j<data.size() && i<num_parameters(); ++j) 
      for (int k=0; k<data[j].clusters.size(); ++k)
    	if (data[j].clusters[k]->load_parameters(modelpars[i])) i += 1;
    return true;
  }
